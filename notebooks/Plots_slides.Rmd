...
(boxplot_claim_frequency <- ggplot(combined_df, aes(x = model, y = Claim_Frequency)) +
  ...
  labs(title = TeX("E(N)"),
       x = "Model",
       y = "") +
 scale_x_discrete(labels = c("Pois(lambda)-Lognormal(mu,sigma=1)" = expression(paste("Lognormal(",mu,",",sigma=1,")")),
                              "Pois(lambda)-Gamma(alpha,beta=1)" = expression(paste("Gamma(",alpha,",",beta=1,")")),
                              "Pois(lambda)-Lognormal(mu=0,sigma)" = expression(paste("Lognormal(",mu=0,",",sigma,")")))) +
  theme_classic()
 +
  theme(
    axis.title = element_text(size = 18),  # Increase font size of axis titles
    axis.text = element_text(size = 18),   # Increase font size of axis text
